...
travel_date = st.sidebar.date_input("눌러서 달력 펼치기", today)
...
if query := st.chat_input("대한민국의 국보 또는 보물을 입력하시거나 물어보세요"):
    ...
    with st.chat_message("user"):
        st.markdown(query)
    st.session_state.messages.append({"질문": query})
    ...
    with st.chat_message("assistant"):
        if response and hasattr(response, 'content') and response.content:
            ...
            st.session_state.messages.append({"답변" : response})
        else:
            st.markdown("죄송합니다. 관련 정보를 찾을 수 없습니다.")


st.sidebar.title("대화 내역")
with st.sidebar:
