...
# csv 파일 로드.
loader_1 = CSVLoader('../크롤링_자료/문화재_크롤링_결과.csv',encoding='UTF8')
loader_2 = CSVLoader('../크롤링_자료/여행지_크롤링_결과.csv',encoding='UTF8')
...
contextual_prompt = ChatPromptTemplate.from_messages([
    ("system", "You will recommend nearby tourist attractions that are in the same city, district, and road name as the location of the specified treasure or national treasure (e.g., the name of the treasure or national treasure). Please include a brief description of each recommended location. Use only the information from the crawled data. Please respond only in Korean. Additionally, if there are no attractions in the exact same district, feel free to provide alternative suggestions in the same city, ensuring your recommendations are relevant to the user's interests. Please answer in Korean."),
...
# 이미지 출력
st.image("../jpg/경복궁.jpg", caption="🤖이런 멋진 문화재.. 주변에 무엇이 있을지 궁금하지 않나요?🤖", use_column_width=True)
...
if query := st.chat_input("대한민국의 국보 또는 보물을 입력하시거나 물어보세요"):
    ...
    # 어시스턴트의 응답을 화면에 표시
        ...
        st.markdown("형식에 맞게 써야 정확한 정보가 나옵니다")
